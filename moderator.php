...
function age_check() {
	...
	*/
	...
	// Check if user creation date is newer than six months ago
	if ($epoch > (time() - 15778800)) {
		return 1;
	// Check if user's join date is newer than one month ago
	} elseif (strtotime($_SESSION['joined_at']) > (time() - 2629800)) {
		return 2;
	}
	return 0;
}
